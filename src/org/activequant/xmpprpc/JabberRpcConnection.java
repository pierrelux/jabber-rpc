...
public class JabberRpcConnection implements Runnable {
    ...
    public JabberRpcConnection(String aUsername, String aServer,
	    String aPassword, String aResource, Role connRole) throws Exception {
	username = aUsername;
	server = aServer;
	password = aPassword;
	resource = aResource;
	...
	System.out.println("Create Jabber RPC connection " + role);
    }

    public JabberRpcConnection(XMPPConnection conn, Role connRole)
	    throws Exception {
	connection = conn;
	...
	System.out.println("Create Jabber RPC from existing connection " + role);
	...
	try {
	    ...
	    System.out.println("Connection role for " + username + " at " + server + " is " + role);
	    System.out.println("Incoming packet of type " + incomingRpcIQ.getType() + "  is " + aPacket.getPayload());
	    
	    if (incomingRpcIQ.getType() == Type.SET) {
		if (role == Role.CLIENT) {
		    System.out
			    .println("Received method call on RPC client connection. Ignoring.");
		    return;
		}
